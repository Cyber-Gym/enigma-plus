...
DATASET_JSON="../NYU_CTF_Bench/development_dataset.json"
...
run_challenge() {
    ...
    tmux new-session -d -s "$session_name" -c "$(pwd)" bash -c "
        ...
        python run.py \\
            ...
            --trajectory_path trajectory/devset_1 \\
            --temperature=0.4 \\
            --top_p=0.95 \\
            --top_k=0 \\
...
if [ -z "$START_INDEX" ]; then
    ...
    # Get all challenge IDs and their paths from the JSON file using null delimiter
    challenges=$(jq -r 'to_entries[] | .key + "\0" + .value.path' "$DATASET_JSON")
    ...
    # Get all challenges first using null delimiter
    all_challenges=$(jq -r 'to_entries[] | .key + "\0" + .value.path' "$DATASET_JSON")
    ...
    challenge_id=$(echo "$challenges" | cut -d$'\0' -f1)
...
instance_id=0
while IFS= read -r -d '' challenge_id && IFS= read -r challenge_path; do
...
done < <(echo "$challenges" | while IFS= read -r line; do
    if [ -n "$line" ]; then
        echo -n "${line%$'\0'*}"
        echo -ne '\0'
        echo "${line#*$'\0'}"
